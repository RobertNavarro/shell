...
TEST(literal_Integration_Test, combinationfailtest) {
	...
	vector<Token*> tokendInput;
   	string fakeUserInput = "test fakefile >> tempTestOuput && (test -d src >> tempTestOuput && test -e src >> tempTestOuput) && test -f src >> tempTestOuput && test -d src >> tempTestOuput && test -e src >> tempTestOuput";
    vector<string> parsedUserInput = parser(fakeUserInput);
	vector<string> separatedInput = separate(parsedUserInput);
	vector<string> mirroredShunting = produceShunting(separatedInput);
...
TEST(literal_Integration_Test, allflags) {
	...
	vector<Token*> tokendInput;
	string fakeUserInput = "test -d unit_tests >> tempTestOuput; test -e unit_tests >> tempTestOuput; test -f unit_tests >> tempTestOuput; test unit_tests >> tempTestOuput";
    vector<string> parsedUserInput = parser(fakeUserInput);
	vector<string> separatedInput = separate(parsedUserInput);
...
TEST(literal_Integration_Test, echotest) {
	...
	vector<Token*> tokendInput;
   	string fakeUserInput = "echo hello >> tempTestOuput && test src >> tempTestOuput";
    vector<string> parsedUserInput = parser(fakeUserInput);
	vector<string> separatedInput = separate(parsedUserInput);
	vector<string> mirroredShunting = produceShunting(separatedInput);
	tokendInput = tokenize(mirroredShunting);
...
TEST(literal_Integration_Test, ortest) {
	...
	vector<Token*> tokendInput;
   	string fakeUserInput = "test -f src >> tempTestOuput || echo hello >> tempTestOuput";
    vector<string> parsedUserInput = parser(fakeUserInput);
	vector<string> separatedInput = separate(parsedUserInput);
	vector<string> mirroredShunting = produceShunting(separatedInput);
	tokendInput = tokenize(mirroredShunting);
