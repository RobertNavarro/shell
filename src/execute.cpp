...
{
	string leftTokenString = leftToken->getWholeToken();
	string rightTokenString = rightToken->getWholeToken();
	...
	FILE* in_pipe = popen(leftTokenString.c_str(), read.c_str());
	FILE* out_pipe = popen(rightTokenString.c_str(), write.c_str());
	if(!in_pipe)
	{
	...
	int leftClose = (pclose(in_pipe)/256);
	int rightClose = (pclose(out_pipe)/256);
	
	if(leftClose != 0 && rightClose != 0)
	{
		//cout << "both sides not equal to zero" << endl;
		return -1;
	}
	else if(leftClose != 0 && rightClose == 0)
	{
		//cout << "left not equal to zero but right is" << endl;
		return 0;
	}
	else if(leftClose == 0 && rightClose != 0)
	{
		//cout << "left equal to zero but right is not" << endl;
		return -1;
	}
	return 0;
